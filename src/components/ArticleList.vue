<script>
import { useLoadArticles, deleteArticle } from "@/firebase";
export default {
  name: "ArticleList",
  setup() {
...
<template>
  <div class="card mt-4 mb-4">
    <table class="table m-0">
      ...
      <tbody>
        <tr
          v-for="{ id, title, authors } in articles.sort(function (a, b) {
        ...
        >
          ...
          <td class="align-middle">
            <router-link :to="`/articleedit/${id}`">
              <button class="btn btn-primary btn-sm me-2 mb-2">Edit</button>
            </router-link>
            <button class="btn btn-danger btn-sm" @click="deleteArticle(id)">
              Delete
            </button>
          </td>
        </tr>
